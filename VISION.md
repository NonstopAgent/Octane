...
4. **No secrets in the client** — integration credentials stay server-side (`GITHUB_TOKEN`, `VERCEL_TOKEN`). Zustand holds connection status, project links, coding job metadata, and display fields only — never tokens.
5. **Coding workbench is review-first** — checkpoint 11C opens PRs (docs + plan) after explicit approval; no auto-merge, deploy, or destructive GitHub actions.
6. **Planning, not professional advice** — legal, tax, investment, and compliance language triggers explicit warnings; outputs are for organizational planning only.
...
| GitHub / Vercel | `/connections`, project detail | Read-only live stats when server tokens configured |
